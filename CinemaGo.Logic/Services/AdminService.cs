...
namespace CinemaGo.Logic.Services
{
    public class AdminService : IAdminService
    {
        ...
        {
            var data = _dBContext.Contacts.ToList();
            List<ContactModel> _contactList = new List<ContactModel>();
            foreach (var c in data)
            {
                ContactModel _contactModel = new ContactModel();
                _contactModel.Id = c.Id;
                _contactModel.FirstName = c.FirstName;
                _contactModel.LastName = c.LastName;
                _contactModel.Email = c.Email;
                _contactModel.PhoneNumber = c.PhoneNumber;
                _contactModel.Comment = c.Comment;
                _contactList.Add(_contactModel);
            }
            return _contactList;
        }

        public bool DeleteContact(ContactModel contactToDelete)
        {
            bool flag = false;
            var _contact = _dBContext.Contacts.Where(x => x.Id == contactToDelete.Id).First();
            if (_contact != null)
            {
                _dBContext.Contacts.Remove(_contact);
                _dBContext.SaveChanges();
                flag = true;
            }
            return flag;
        ...
        {
            var data = _dBContext.OrderDetails.ToList();
            List<OrderDetailModel> _orderdetailList = new List<OrderDetailModel>();
            foreach (var c in data)
            {
                OrderDetailModel _orderdetailModel = new OrderDetailModel();
                _orderdetailModel.Id = c.Id;
                _orderdetailModel.OrderId = c.OrderId;
                _orderdetailModel.ProductId = c.ProductId;
                _orderdetailModel.Quantity = c.Quantity;
                _orderdetailModel.SubTotal = c.SubTotal;
                _orderdetailModel.CreatedOn = c.CreatedOn;
                _orderdetailList.Add(_orderdetailModel);
            }
            return _orderdetailList;
        }

        public bool DeleteOrderDetail(OrderDetailModel orderdetailToDelete)
        {
            bool flag = false;
            var _orderdetail = _dBContext.OrderDetails.Where(x => x.Id == orderdetailToDelete.Id).First();
            if (_orderdetail != null)
            {
                _dBContext.OrderDetails.Remove(_orderdetail);
                _dBContext.SaveChanges();
                flag = true;
            }
            return flag;
